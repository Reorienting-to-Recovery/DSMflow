...
The table below shows monthly flow CFS for a wet year model CSF on the San Joaquin River. This table was constructed using the Natural Flows database and model survival thresholds.
...
sj_functional_flows_for_model_wet <- tibble(month = factor(month.abb[1:12], 
                           ...
                           flow_cfs = c(45500, # Jan 
                                        ...
                                        45500, # March 
                                        45500, # April 
                                        6553, # May Spring recession flow (6,553 for mig surv ~50%)
                                        6553, # June Spring recession flow (6,553 for mig surv ~50%)
                                        6553, # July (6,553 for mig surv ~50%)
...
```{r, echo = FALSE}
combined_data <- bind_rows((sj_functional_flows_for_model_wet |> mutate(Type = "Model CSF Wet Year")), 
...
sj_functional_flows_for_model_dry <- tibble(month = factor(month.abb[1:12], 
                                                        levels = month.abb[c(10, 11, 12, 1:9)]),
                           # month_name = month.abb[month],
                           flow_cfs = c(2000, # Jan Wet season base flow + Peak Magnitude flows, based on dry year monthly Natural Flows 
                                        2000, # Feb Wet season base flow + Peak Magnitude flows, based on dry year monthly Natural Flows 
                                        2000, # March Wet season base flow, based on dry year monthly Natural Flows 
                                        2000, # April Wet season base flow, based on dry year monthly Natural Flows 
                                        1500, # May Spring recession flow (keep migratory surv at ~20%)
                                        1500, # June Spring recession flow (keep migratory surv at ~20%)
                                        1500, # July half way between dry season baseflow and mig surv 20% threshold
                                        1200, # Aug - dry season baseflow 1 (keep migratory surv at ~20%), 
                                        280, # Sept - All fall runs juves have left system, use Natural Flows dry season base flow
                                        400, # oct - dry season baseflow, add some pulse flow in 
                                        400, # Nov add fall pulse flow, based on dry year monthly Natural Flows
                                        400 # Dec Begin dry season baseflow and medium flow, based on dry year monthly Natural Flows
                                         ),
                            description = c("Utilize remaining water for FP habitat", # Try for fp rearing hab, less important - dependent on acre ft 
                                           "Utilize remaining water for FP habitat", # Optimize for  fp rearing habitat 
                                           "Utilize remaining water for FP habitat", # Optimize for  fp rearing habitat 
                                           ...
                                           "Set summer base flow to 1200 based on Natural flows tool and to incrase acreage for avaliable for Juv flows",
                                           "Optimize for spawning habitat based on flow area WUA ", # Optimize for spawning habitat based on flow area WUA 
                                           "Optimize for spawning habitat based on flow area WUA ", # Optimize for spawning habitat 
                                           "Optimize for spawning habitat based on flow area WUA ")) # Optimize for spawning habitat 
...
The plot below shows the CalSIM modeled flows for the Sacramento River of an existing representative dry year: 1959. 1959 has the median water year index as provided by DWR out of all the "Critical", "Dry", and "Below Normal" years within our model time frame.


```{r, echo = FALSE}
combined_data_dry <- bind_rows((sj_functional_flows_for_model_dry |> mutate(Type = "Model CSF Wet Year")), 
