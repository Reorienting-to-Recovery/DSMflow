...
# Node C614
names <- read_csv("data-raw/calsim_2019_BiOp_ITP/C422_C843.csv", skip = 1) |> names()
C614 <- read_csv("data-raw/calsim_2019_BiOp_ITP/C422_C843.csv", skip = 7, 
                 col_names = names) |> 
  rename(date = `...2`) |> 
...
DSMflow::flows_cfs$biop_itp_2018_2019 |> 
  ...
  mutate(flow_relationship_original = `San Joaquin River` / `Upper-mid Sacramento River`,
         flow_relationship_C614 = C614 / `Upper-mid Sacramento River`) |> 
  pivot_longer(`Upper-mid Sacramento River`:`C614`,
...
scale_thresholds <- DSMflow::flows_cfs$biop_itp_2018_2019 |> 
  ...
  mutate(flow_relationship_SJ = `San Joaquin River` / `Upper-mid Sacramento River`,
         flow_relationship_C614 = C614 / `Upper-mid Sacramento River`) |>
  pivot_longer(flow_relationship_SJ:flow_relationship_C614,
               names_to = "node",
               values_to = "ratio") 
  # pivot_longer(`Upper-mid Sacramento River`:`C614`,
  #              names_to = "watershed", values_to = "flow_cfs") |> 
  # filter(watershed %in% c("San Joaquin River", "C614"))
...
flow_threshold_50 <- flow_to_survival_thresholds |> 
  ...
  pull(`San Joaquin Flow Threshold`)
...
sj_functional_flows_for_model_wet <- tibble(month = factor(month.abb[1:12], 
                           ...
                           description = c("Utilize remaining water for FP habitat", # Try for fp rearing hab, less important - dependent on acre ft 
                                           ...
                                           "Utilize remaining water for FP habitat", # Optimize for  fp rearing habitat 
                                           "Utilize remaining water for FP habitat", # Optimize for fp rearing habitat 
                                           "Optimize for migratory survival (3,258 for mig surv ~50%)", # 
                                           "Optimize for migratory survival (3,258 for mig surv ~50%)", # Optimize for migratory survival (keep as high as possible but above mig surv thresholds)
                                           "Optimize for migratory survival (3,258 for mig surv ~50%)", # Optimize for migratory survival (keep as high as possible but above mig surv thresholds)
...
sj_functional_flows_for_model_dry <- tibble(month = factor(month.abb[1:12], 
                           ...
                           flow_cfs = c(5000, # Jan Wet season base flow + Peak Magnitude flows, based on dry year monthly Natural Flows 
                            ...
                            description = c("Utilize remaining water for FP habitat", # Try for fp rearing hab, less important - dependent on acre ft 
                                           ...
                                           "Utilize remaining water for FP habitat", # Optimize for  fp rearing habitat 
                                           "Utilize remaining water for FP habitat", # Optimize for fp rearing habitat 
                                           "Keep high for migratory survival (1,526 for mig surv ~20%)", # 
                                           "Keep high for migratory survival (1,526 for mig surv ~20%)", # Optimize for migratory survival (keep as high as possible but above mig surv thresholds)
                                           "Keep high for migratory survival (1,526 for mig surv ~20%)", # Optimize for migratory survival (keep as high as possible but above mig surv thresholds)
