...
The Reorienting to Recovery Structured Decision Making group proposed a effective functional flow (EFF) scenario. To date, there is no available CalSIM EFF run so we cannot model full EFF on the entire system. This document outlines our approach to piloting an EFF on the Sacramento River constructed to maximize salmonid survival. 

We call this modified EFF approach a Constructed Salmonid Flow (CSF).

**Pilot CSF Approach on the Sacramento River**

The R2R technical team is worked collaboratively with a flow subgroup to develop a CSF hydrograph for the Sacramento River. We built off of the Nature Conservancy's Natural Flows Database to develop a wet and dry year monthly EFF hydrograph and compiled these into a 20 year synthetic time series based on the historical water year types represented in the R2R model time frame (1980-2000). 
...
-   The technical team is also working with MBK to incorperate EFF logic into a CalSIM Run that we can use directly in the model.

- The technical team is working to apply the rulesets for the Sacramento CSF to develop a San Joaquin CSF time series.
...
EFF are hypothesized to be the best flows for salmon in our historical system. However, our system is highly managed so altered EFF slightly from their historical values to maximize benefit to salmon (creating what we are calling Constructed Salmonid Flows).

The R2R techncial team developed proposed CSF monthly flows by combining the [Natural Flows Database](https://ceff.ucdavis.edu/sites/g/files/dgvnsk5566/files/media/documents/CEFF%20Technical%20Report%20Ver%201.0%20Mar_31_2021_DRAFT_FINAL%20for%20web.pdf) values with critical flow to survival migratory thresholds on the Sacramento River (from [Michel 2021](https://esajournals.onlinelibrary.wiley.com/doi/full/10.1002/ecs2.3498)). 
...
1) Optimize spawning habitat during full spawning time frame (October, November, and December)
