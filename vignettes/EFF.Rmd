...
water_year_types <- waterYearType::water_year_indices |> 
  filter(location == "Sacramento Valley", WY %in% c(1922:2000)) |> 
...
synthetic_eff_sac <- bind_rows(wet_years, dry_years) |> 
  mutate(date = as_date(paste0(year, "-", month, "-", lubridate::days_in_month(month)))) |> 
  select(-year, -year_type, -month, -description, "Upper Sacramento River EFF" = flow_cfs, date) |> glimpse()
